...
public class PrimitiveAlloyStationMenu extends AbstractContainerMenu {
    ...
    // Store fluid info for client-side
    private final List<Fluid> registeredFluids = new ArrayList<>();
    ...
    public PrimitiveAlloyStationMenu(int containerId, Inventory playerInventory, PrimitiveAlloyStationBlockEntity blockEntity, ContainerData data) {
        ...
        // Cache registered fluids for client syncing
        for (FluidTank tank : blockEntity.getAllTanks()) {
            Fluid fluid = getValidFluidFromTank(tank);
    ...
    // Helper to extract valid fluid from tank
    private Fluid getValidFluidFromTank(FluidTank tank) {
        // First check if tank already has fluid
        if (!tank.getFluid().isEmpty()) {
            return tank.getFluid().getFluid();
        }

        // Try to identify tank by checking which fluid it accepts
        for (String fluidName : SmeltableFluidsRegistry.getAllFluidNames()) {
            Fluid fluid = ModFluids.getSourceFluid(fluidName);
            if (fluid != null && tank.isFluidValid(new FluidStack(fluid, 1))) {
                return fluid;
            }
        }
        return Fluids.EMPTY;
    ...
    public void setData(int id, int data) {
        ...
        // Skip progress indices (0, 1)
        if (id < 2) return;

        // Calculate tank index
        int tankIndex = id - 2;
        FluidTank tank = blockEntity.getTankByIndex(tankIndex);

        if (tank != null) {
            // Get the corresponding fluid type
            Fluid fluid = (tankIndex < registeredFluids.size())
                    ? registeredFluids.get(tankIndex)
                    : Fluids.EMPTY;

            if (fluid != Fluids.EMPTY) {
                tank.setFluid(new FluidStack(fluid, data));
            }
        }
    }
    ...
    /**
     * Get all fluid layers for rendering (DYNAMIC)
     */
    public List<FluidLayer> getFluidLayers() {
        if (blockEntity == null) {
            return Collections.emptyList();
        }
        return blockEntity.getFluidLayers();
    }

    /**
     * Get total capacity for the combined tank display
     */
    public int getTotalCapacity() {
        if (blockEntity == null) {
            return 1440 * 5; // Fallback
        }
        return blockEntity.getTotalCapacity();
    }
    ...
    public void debugFluidAmounts() {
        ...
        System.out.println("[Menu Debug] Total tanks: " + blockEntity.getAllTanks().size());
        for (int i = 0; i < blockEntity.getAllTanks().size(); i++) {
            FluidTank tank = blockEntity.getTankByIndex(i);
            if (tank != null && !tank.getFluid().isEmpty()) {
                System.out.println("  Tank " + i + ": " + tank.getFluid().getDisplayName().getString() +
